...
イメージ0～3は同じVID、PID、Manufacturer、Product、Serial Numberを使用します。実行中の
...
Capability bitは`0x01` BOOT、`0x02` FLASH、`0x04` UARTです。イメージ0は`0x03`、
イメージ1～3は`0x05`を返し、予約bitはゼロです。

### SELECT_IMAGE `0x01`

リクエストpayloadは対象イメージ`0`～`3`の1バイトです。レスポンス・オペコードは`0x81`、
payloadはBOOT statusの1バイトです。
...
リクエストpayloadはスロット`1`、`2`、`3`の1バイトです。レスポンス・オペコードは`0x81`で、
statusを1バイト返します。成功時は256 KiBのスロット全体を消去し、WIPクリア後に応答します。
イメージ0は常に保護します。
...
イメージ1～3（`0x040000-0x0fffff`）に限定し、イメージ0はハードウェアで保護します。
...
各256 KiBユーザースロットの最後の32バイトをマニフェストに使用します。生の
...
書き込み前にGET_INFOを送ります。ユーザーイメージが動作中ならイメージ0を要求し、同じ
Serial Numberの再列挙を待ち、GET_INFOでイメージ0を確認してからerase、write、manifest、
readbackを実行します。
...
Webページからもデバイス側保護は迂回できません。イメージ0は書き込み不可で、FPGAが
